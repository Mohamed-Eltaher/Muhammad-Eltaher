...
//		add_action( 'wp_ajax_wcpr_search_product', array( $this, 'search_product' ) );
			...
			if ( count( $comments ) ) {
				foreach ( $comments as $comment ) {
					...
					if ( $user_id ) {
						...
						if ( ! in_array( $product_id, $user_coupon ) ) {
	...
	function preview_emails_html() {
		global $pagenow;
		if ( $pagenow == 'admin.php' && isset( $_REQUEST['page'] ) && $_REQUEST['page'] == 'woo-photo-reviews' ) {
			?>
		...
		if ( $pagenow == 'admin.php' && isset( $_REQUEST['page'] ) && $_REQUEST['page'] == 'woo-photo-reviews' ) {
			...
			if ( in_array( $editor_id, $editor_ids ) ) {
                ...
                <span class="button coupon-preview-emails-button"
                      data-wcpr_language="<?php echo str_replace( 'content', '', $editor_id ) ?>"><?php esc_html_e( 'Preview emails', 'woo-photo-reviews' ) ?></span>
				<?php
				echo ob_get_clean();
			}
			...
			if ( in_array( $editor_id, $editor_ids ) ) {
                ...
                <span class="button reminder-preview-emails-button"
                      data-wcpr_language="<?php echo str_replace( 'follow_up_email_content', '', $editor_id ) ?>"><?php esc_html_e( 'Preview emails', 'woo-photo-reviews' ) ?></span>
				<?php
				echo ob_get_clean();
			}
		}
	...
	public function preview_emails_ajax() {
		...
		if ( $email_type == 'coupon' ) {
			...
			$content       = str_replace( '{date_expires}', date_i18n( 'F d, Y', $date_expires ), $content );
			...
			$order_id               = 1;
			$now                    = strtotime( 'now' );
			$date_create            = date_i18n( 'F d, Y', $now - 86400 );
			$date_complete          = date_i18n( 'F d, Y', $now );
			$content                = str_replace( '{customer_name}', $customer_name, $content );
            ...
            <form action="" method="POST" class="vi-ui form">
                        ...
                        <tr>
                            ...
                            <td>
                                <a class="vi-ui button" target="_blank"
                                   href="http://bit.ly/woocommerce-photo-reviews"><?php esc_html_e( 'Upgrade This Feature', 'woo-photo-reviews' ) ?></a>
                            </td>
                        </tr>
                ...
                <div class="vi-ui bottom tab segment" data-tab="photo">
                    <table class="form-table">
                            ...
                            <td>
                                <div class="inline field">
                                    <input id="image_maxsize" class="kt-photo-reviews-setting" type="number"
                                           ...
                                           max="<?php echo absint( 1024 * absint( ini_get( 'upload_max_filesize' ) ) ); ?>"
                                           step="1"
                                           value="<?php echo $this->settings->get_params( 'photo', 'maxsize' ); ?>"><?php esc_html_e( ' kB (Max ' . absint( 1024 * absint( ini_get( 'upload_max_filesize' ) ) ) . 'kB).', 'woo-photo-reviews' ) ?>
                                </div>
                        ...
                        <tr class="masonry-options <?php echo $this->settings->get_params( 'photo', 'display' ) == 2 ? "wcpr-hidden-items" : '' ?>">
                            <th>
                        ...
                        <tr class="default-options <?php echo $this->settings->get_params( 'photo', 'display' ) == 1 ? "wcpr-hidden-items" : '' ?>">
                            <th>
                        ...
                        <tr>
                            ...
                            <td>
                                <input id="kt_coupons_if_min_rating" type="number" name="kt_coupons_if_min_rating"
                                       placeholder="0" min="0"
                                       max="5" step="1"
                                       value="<?php echo isset( $this->settings->get_params( 'coupon', 'require' )['min_rating'] ) ? $this->settings->get_params( 'coupon', 'require' )['min_rating'] : '' ?>"><?php esc_html_e( 'Stars', 'woo-photo-reviews' ) ?>
                            </td>
                        </tr>
                        <tr>
                            <th>
                        ...
                        <tr class="kt-custom-coupon">
                            ...
                            <td>
                                <input type="number" min="0" name="kt_expiry_date" id="kt_expiry_date"
                                       value="<?php echo $coupon_generate['expiry_date']; ?>"><?php esc_html_e( 'Days', 'woo-photo-reviews' ) ?>
                            </td>
                        </tr>
                        <tr class="kt-custom-coupon">
                            <th>
	...
	public function wcpr_add_meta_box() {
		add_meta_box(
			'wcpr-comment-photos', __( 'Photo' ), array(
			$this,
	...
	public function add_columns( $cols ) {
		$cols['wcpr_photos'] = __( 'Photos', 'woo-photo-reviews' );
		$cols['wcpr_rating'] = __( 'Rating', 'woo-photo-reviews' );
	...
	public function add_menu() {
		...
		add_submenu_page(
			'woo-photo-reviews', __( 'Status', 'woo-photo-reviews' ), __( 'System Status', 'woo-photo-reviews' ), 'manage_options', 'kt-wcpr-status', array(
				$this,
				'status'
			)
		);
	...
	public function search_parent_product( $x = '', $post_types = array( 'product' ) ) {
		if ( ! current_user_can( 'manage_options' ) ) {
			return;
		}
		ob_start();
	...
	public function search_product( $post_types = array( 'product' ) ) {
		if ( ! current_user_can( 'manage_options' ) ) {
			return;
		}
		ob_start();
	...
	public function search_coupon( $post_types = array( 'shop_coupon' ) ) {
		if ( ! current_user_can( 'manage_options' ) ) {
			return;
		}
		ob_start();
